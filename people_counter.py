...
TEAM ILLUMINATI 
Prabhav Singh, Ridam Srvastava, Aryan Ganjoo, Swati Garg

WE HAVE TRIED TO COMMENT AND DESCRIBE MOST OF THE CODE
PLEASE REACH OUT TO US IN CASE OF ANY PROBLEMS
prabhavsingh55221@gmail.com
...
This model is implemented using the Caffe framework. For details about this model, check out the repository for the same.
The model input is a blob that consists of a single image of 1x3x300x300 in BGR order. 
The BGR mean values need to be subtracted as follows: [127.5, 127.5, 127.5] before passing the image blob into the network. In addition, values must be divided by 0.007843.
...
''' INITIALISING INPUT - EITHER WEBCAM (CCTV for live deployment) OR VIDEO '''
...
''' Setting up frame and CENTROID TRACKER (Described in Docs) '''
...
''' LOOPING OVER FRAMES IN VIDEO (34 FPS) '''
	...
	if totalFrames % args["skip_frames"] == 0:
		...
		for i in np.arange(0, detections.shape[2]):
			...
			if confidence > args["confidence"]:
				...
				# if the class label is not a person, ignore it (IMPORTANT)
